...
class MessageNotificationService {
  ...
  Future<bool> initialize() async {
    ...
    _isInitialized = true;
    debugPrint(
        'Message notification service initialized, user ID: $_currentUserId');
    return true;
  }
  ...
  Future<void> _loadLastMessageTimestamps() async {
    try {
      ...
      debugPrint(
          'Loaded ${_lastMessageTimestamps.length} last message timestamps');
    } catch (e) {
  ...
  Future<void> _saveLastMessageTimestamp(
      String conversationId, int timestamp) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt('last_msg_ts_$conversationId', timestamp);
      debugPrint(
          'Saved last message timestamp for conversation $conversationId: $timestamp');
    } catch (e) {
  ...
  Future<void> subscribeToConversation(
      String conversationId, String contactName) async {
    await initialize();
    ...
    debugPrint(
        'Subscribing to notifications for conversation: $conversationId');
    ...
    final subscription =
        messagesRef.limitToLast(1).onChildAdded.listen((event) async {
      // Check if app is in foreground before proceeding
      // This is crucial to ensure we don't double-notify when the app is in the background
      if (_appState != AppLifecycleState.resumed) {
        debugPrint(
            '[MessageNotificationService] App is in ${_appState.toString()}, NOT showing database notification');
        return;
      }
      ...
      try {
        ...
        // Add debugging logs
        final messageId = messageData['id']?.toString() ?? 'unknown';
        debugPrint(
            '[MessageNotificationService] Firebase DB message received: $messageId');
  ...
  Future<void> _processMessage(String conversationId, String contactName,
      Map<dynamic, dynamic> messageData) async {
    // NOTE: Log which handler is processing the message
    final messageId = messageData['id']?.toString() ?? 'unknown';
    debugPrint(
        '[MessageNotificationService] Processing message: $messageId in conversation: $conversationId');

    // Double-check app state again
    if (_appState != AppLifecycleState.resumed) {
      debugPrint(
          '[MessageNotificationService] App NOT in foreground, blocking notification');
      return;
    }
    ...
    if (timestamp <= lastTimestamp) {
      debugPrint(
          '[MessageNotificationService] Skipping notification for old or duplicate message: $messageId');
      return;
    }

    // Check shared preferences to see if FCM already handled this message
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      if (lastNotificationData != null) {
        try {
          ...
          final lastTimestampFromPref = lastData['timestamp'] as int?;
          if (lastTimestampFromPref != null) {
            final now = DateTime.now().millisecondsSinceEpoch;

            // Use a shorter threshold for foreground notifications
            final timeThreshold =
                _appState == AppLifecycleState.resumed ? 500 : 5000;

            // Use different thresholds based on app state
            if (now - lastTimestampFromPref < timeThreshold) {
              debugPrint(
                  '[MessageNotificationService] Recent notification for this conversation within ${timeThreshold}ms, skipping');
              return;
            }
          }
        } catch (e) {
      ...
      // Check if FCM already processed this message
      final processedMessageIds =
          prefs.getStringList('processed_message_ids') ?? [];

      if (processedMessageIds.contains(messageId)) {
        debugPrint(
            '[MessageNotificationService] Message $messageId was already processed by FCM, skipping');
        return;
      }
    } catch (e) {
      debugPrint(
          '[MessageNotificationService] Error checking processed messages: $e');
    }
    ...
    String messageContent =
        TextUtils.fixArabicEncoding(messageData['content'] ?? 'رسالة جديدة');
    ...
    debugPrint(
        'Notification sent for message from $senderName in conversation $conversationId');
  }
  ...
  void unsubscribeFromConversation(String conversationId) {
    ...
    if (subscription != null) {
      ...
      debugPrint(
          'Unsubscribed from notifications for conversation: $conversationId');
    }
  }
